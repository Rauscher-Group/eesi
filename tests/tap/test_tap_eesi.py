...
N, D = 6, 3
# `_anchored` only needs a valid parameter set -- these tests are about the
# interpolant's geometry, not the prior's law.
K, B_LEN, GAMMA, COS0 = 5.0, 1.0, 2.0, 0.5
...
def _anchored(B: int, seed: int = 0) -> torch.Tensor:
    return sample_prior(B, K, B_LEN, GAMMA, COS0, n_particles=N, n_dims=D,
